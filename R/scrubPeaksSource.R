...
scrubPeaksSource <- function(df,
  ...
  if (!is.null(iniPico) && is.null(finPico) && !is.null(sourceToScrub)){
    rlang::warn(paste0("You provided only the start of the peak, I will assume that you meant everything after ", iniPico, ", from medium", sourceToScrub))
    df <- df %>% filter(createdAt >= iniPico & params.utm_medium != sourceToScrub)
  }
  if (!is.null(iniPico) && is.null(finPico) && is.null(sourceToScrub)){
  ...
  if (is.null(iniPico) && is.null(finPico) && !is.null(sourceToScrub)){
    rlang::warn(paste0("You provided only medium, I will scrub all rows from medium ", sourceToScrub))
    df <- df %>% filter(params.utm_medium != sourceToScrub)
  }
  if (!is.null(iniPico) && !is.null(finPico) && !is.null(sourceToScrub)){
    rlang::inform(paste0("Peak starting at:",iniPico,", ending at ", finPico, " from medium ", sourceToScrub ))
    df <- df %>% filter(!((createdAt >= iniPico & createdAt <= finPico) & params.utm_medium == sourceToScrub))
  }
  # saving attributes
